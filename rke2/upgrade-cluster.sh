...
do
  ...
  then
    ...
    eval "$(echo "ssh -i ${ssh_key} root@${node_ip[$c]} rke2/scripts/upgrade_master.sh")"
  fi
done
...
do
  ...
  then
    ...
    eval "$(echo "ssh -i ${ssh_key} root@${node_ip[$c]} rke2/scripts/upgrade_worker.sh")"
  else
    echo "${node_name[$c]} :" "master"
