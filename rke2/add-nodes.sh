...
do
  ...
  eval "$(echo "ssh -i ${ssh_key} root@${node_ip[$c]} chmod +x rke2/scripts/*.sh")"
  ...
  then
    ...
    eval "$(echo "ssh -i ${ssh_key} root@${node_ip[$c]} rke2/scripts/master_init.sh")"
  fi
done
...
do
  ...
  then
    ...
    eval "$(echo "ssh -i ${ssh_key} root@${node_ip[$c]} rke2/scripts/master_member.sh")"
  elif [[ "${node_role[$c]}" == "worker" ]]
  then
    # 노드 역할이 worker 인 경우 worker.sh 실행
    echo "${node_name[$c]} :" "worker"
    echo "====== SETTING WORKER UP ======"
    eval "$(echo "ssh -i ${ssh_key} root@${node_ip[$c]} rke2/scripts/worker.sh")"
  else
    echo "${node_name[$c]} :" "master"
